...
func (sl standLocator) handleApiStands(w http.ResponseWriter, r *http.Request) { //req handler
	...
	err := dec.Decode(&loc) //calling decode method on decoder
	if err != nil {
		log.Println(fmt.Errorf("Error retrieving location: %v", err)) //log out err
		enc := json.NewEncoder(w)
		enc.Encode([]viewmodel.StandCoordinate{}) // new encoder to send back an empty slice
		return
	}
